...
fn eval_failure_discards_path() {
    ...
    assert!(!has_result, "failed function must discard the substitution");
}
...
#[test]
fn non_numeric_return_binds_symbol() {
    let spl = r#"
        (given (val 5))
        (normally r1
    ...
    let result = prepare(&theory, opts).expect("prepare should bind the returned symbol");
    ...
    assert!(
        has_result,
        "should produce a result with the returned symbol"
    );
}
...
#[test]
fn default_pipeline_supplies_builtin_registry() {
    let theory = parse_spl("(normally r (bind ?n (+ 1 2)) (n ?n))").unwrap();
    let (prepared, _) = spindle_core::pipeline::Pipeline::default_pipeline()
        .run(theory)
        .unwrap();
    assert!(
        prepared
            .rules()
            .any(|r| r.head.iter().any(|h| h.to_spl() == "(n 3)"))
    );
}
