...
fn reason_spl(input: &str) -> Vec<String> {
    ...
    let conclusions = reason_with_options(&theory, PrepareOptions::default()).expect("reason failed");
...
fn fold_sum_over_base_facts() {
    let conclusions = reason_spl(r#"
        ...
        (normally r-total
            ...
            (total-pay ?emp ?total))
    "#);
    assert!(has_conclusion(&conclusions, "+d total-pay(alice, 55)"), "got: {conclusions:?}");
    assert!(has_conclusion(&conclusions, "+d total-pay(bob, 40)"), "got: {conclusions:?}");
}
...
fn fold_count_over_base_facts() {
    let conclusions = reason_spl(r#"
        ...
        (normally r-count
            ...
            (shift-count ?emp ?n))
    "#);
    assert!(has_conclusion(&conclusions, "+d shift-count(alice, 3)"), "got: {conclusions:?}");
    assert!(has_conclusion(&conclusions, "+d shift-count(bob, 1)"), "got: {conclusions:?}");
}
...
fn fold_min_over_base_facts() {
    let conclusions = reason_spl(r#"
        ...
        (normally r-min
            ...
            (min-rate ?emp ?m))
    "#);
    assert!(has_conclusion(&conclusions, "+d min-rate(alice, 25)"), "got: {conclusions:?}");
}
...
fn fold_max_over_base_facts() {
    let conclusions = reason_spl(r#"
        ...
        (normally r-max
            ...
            (max-rate ?emp ?m))
    "#);
    assert!(has_conclusion(&conclusions, "+d max-rate(alice, 40)"), "got: {conclusions:?}");
}
...
fn fold_with_identity_empty_set_fires() {
    ...
    let conclusions = reason_spl(r#"
        ...
        (normally r-total
            ...
            (total-pay ?emp ?total))
    "#);
    assert!(has_conclusion(&conclusions, "+d total-pay(alice, 0)"), "got: {conclusions:?}");
}
...
fn fold_extract_expression() {
    ...
    assert!(has_conclusion(&conclusions, "+d daily-pay(alice, 380)"), "got: {conclusions:?}");
}
...
fn fold_over_derived_relation() {
    ...
    assert!(has_conclusion(&conclusions, "+d total-pay(alice, 350)"), "got: {conclusions:?}");
}
...
fn award_scenario() {
    ...
    // bob: 8*30 = 240
    assert!(has_conclusion(&conclusions, "+d daily-pay(alice, 350)"), "got: {conclusions:?}");
    assert!(has_conclusion(&conclusions, "+d daily-pay(bob, 240)"), "got: {conclusions:?}");
}
...
fn regression_basic_defeasible_no_fold() {
    ...
    "#);
    assert!(has_conclusion(&conclusions, "+d flies"), "got: {conclusions:?}");
}
...
fn regression_grounding_no_fold() {
    ...
    "#);
    assert!(has_conclusion(&conclusions, "+D ancestor(alice, carol)"), "got: {conclusions:?}");
}
