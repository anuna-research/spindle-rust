...
// ---------------------------------------------------------------------------
// Module-level tests to verify each fixture builds correctly
// ---------------------------------------------------------------------------
...
mod tests {
    ...
    #[test]
    fn test_tweety_triangle_structure() {
        let t = tweety_triangle();
    ...
    #[test]
    fn test_nixon_diamond_structure() {
        let t = nixon_diamond();
    ...
    #[test]
    fn test_inheritance_chain_depth_zero() {
        let t = inheritance_chain(0);
        // Just the base fact
    ...
    #[test]
    fn test_inheritance_chain_depth_five() {
        let t = inheritance_chain(5);
        // 1 fact + 5 rules = 6
    ...
    #[test]
    fn test_inheritance_chain_depth_one() {
        let t = inheritance_chain(1);
        // 1 fact + 1 rule = 2
        assert_eq!(t.rule_count(), 2);
    }

    #[test]
    fn test_temporal_theory_structure() {
        let t = temporal_theory();
        // 2 facts (employee, on_leave) + 3 rules (t_r1 via add_defeasible_rule, t_r2, t_r3) = 5
        // But r1 is auto-labeled, so total: employee fact, on_leave fact, r1, t_r2, t_r3
        assert_eq!(t.rule_count(), 5);
        assert_eq!(t.superiorities().len(), 1);

        // Verify temporal bounds on on_leave fact
        let on_leave = t.get_rule("t_f1").expect("on_leave fact should exist");
        assert!(on_leave.head_literal().temporal.has_info());

        // Verify temporal bounds on r2
        let r2 = t.get_rule("t_r2").expect("t_r2 should exist");
        assert!(r2.temporal.has_info());

        // Verify temporal bounds on r3
        let r3 = t.get_rule("t_r3").expect("t_r3 should exist");
        assert!(r3.temporal.has_info());
    }

    #[test]
    fn test_modal_theory_structure() {
        let t = modal_theory();
        // 2 facts + 5 modal rules = 7
        assert_eq!(t.rule_count(), 7);
        assert_eq!(t.facts().count(), 2);

        // Verify modal operators are set
    ...
    #[test]
    fn test_conflicting_defeaters_structure() {
        let t = conflicting_defeaters();
        // 3 facts + 2 defeasible + 3 defeaters = 8
    ...
    #[test]
    fn test_empty_theory_structure() {
        let t = empty_theory();
